...
gate checks that those tests are not shallow.
...
`mint` runs the generated unit tests and conformance tests under Python's stdlib
`trace` module, in separate subprocesses, then merges covered line numbers for files
